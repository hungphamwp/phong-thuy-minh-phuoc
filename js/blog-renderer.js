...
class BlogRenderer {
    ...
    getPublishedPosts() {
        if (typeof blogManager !== 'undefined') {
            return blogManager.getPublishedPosts();
        }
        // Fallback
    ...
    renderBlogGrid(category = 'Tất Cả', page = 1) {
    ...
    renderPagination(totalPosts) {
        ...
        if (this.currentPage > 1) {
            ...
            prevLink.addEventListener('click', (e) => {
                e.preventDefault();
                this.renderBlogGrid(this.currentCategory, this.currentPage - 1);
        ...
        for (let i = 1; i <= totalPages; i++) {
            if (i === 1 || i === totalPages || (i >= this.currentPage - 1 && i <= this.currentPage + 1)) {
                ...
                pageLink.addEventListener('click', (e) => {
                    e.preventDefault();
                    this.renderBlogGrid(this.currentCategory, i);
                    window.scrollTo({ top: 0, behavior: 'smooth' });
                });
    ...
    setupCategoryFilter() {
        ...
        filterButtons.forEach(btn => {
            btn.addEventListener('click', function () {
                ...
                blogRenderer.renderBlogGrid(category, 1);
            });
        });
    }
